...
def double_guassian(x1, x2, a1, a2, u1, u2, s1, s2):
    g1 = a1*abs(x1[1] - x1[0])*np.exp(-((x1-u1)**2)/(2*s1**2))/(np.sqrt(2*np.pi*s1**2))
    g2 = a1*abs(x2[1] - x2[0])*np.exp(-((x2-u2)**2)/(2*s2**2))/(np.sqrt(2*np.pi*s2**2))
    return g1 + g2
...
def convolve(E,res,nbins,max_range,fig1,s):
    ...
    y2 = signal.fftconvolve(ym2,filt2, 'same')*norm2
    y3 = signal.fftconvolve(ym3,filt3,'same')*norm3
    ...
    # ax1.plot(xm2, signal.fftconvolve(ym1, filt1, 'same')*norm2, '-', color='red')
    # ax2.plot(xm2, signal.fftconvolve(ym2, filt2, 'same')*norm2, '-', color='red')
    ...
    ax1.set_xlim([0,2])
    ax1.set_title(f'Resolution = {res[1]} eV')
    ax1.set_xlabel('Final Particle Energy (keV)')
    ax1.set_xlim([0,1])
    ax1.set_ylim([0,4])
...
def detector(n_decays,source_displacement,geometry,isotope,material,res,nbins,max_range):
    ...
    # fig4 = plt.figure(4)
    # n,b = convolve(plate2,res,nbins,max_range,fig4,s)
    # curve_fit(n[1],b[1],res[1],fig4)
    plt.show()
    # return n,b,nbins,max_range
...
# angle dependent so energy maximal lost can be estimated from maximal anlge
